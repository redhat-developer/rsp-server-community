...
public class FelixVersions {
    ...
    private static void ensureVersionAdded(String singleAddition, JsonObject serversGson) {
        ...
        String serverTypeKey = "org.jboss.ide.eclipse.as.server.felix." + majorMinor(singleAddition);
        ...
        if( typeObj == null ) {
            ...
            generatedType.addProperty("template", "felix.template");
            JsonObject generatedDiscoveries = new JsonObject();
            String majorDiscoveryKey = "apache.felix." + major(singleAddition) + "x";
            JsonObject majorDiscoveryVal = new JsonObject();
            majorDiscoveryVal.addProperty("discoveryType", "jarManifest");
            majorDiscoveryVal.addProperty("name", "Apache Felix " + major(singleAddition) + ".x");
            majorDiscoveryVal.addProperty("nameFile", "bin/felix.jar");
            majorDiscoveryVal.addProperty("nameKey", "Bundle-Name");
            majorDiscoveryVal.addProperty("nameRequiredPrefix", "Apache Felix");
            majorDiscoveryVal.addProperty("versionFile", "bin/felix.jar");
            ...
            generatedDownloads.addProperty("downloadProviderId", "felix" + major(singleAddition) + "x.download");
            generatedType.add("downloads", generatedDownloads);

            JsonObject generatedInnerType = new JsonObject();
            generatedInnerType.addProperty("name", "Apache Felix " + major(singleAddition) + ".x");
            String descString = "A server adapter capable of discovering and controlling an Apache Felix " + major(singleAddition) + ".x OSGi Container.";
        ...
        String dlKey = "felix-" + singleAddition;
        if( downloadsForType.get(dlKey) == null ) {
            ...
            if( size > -1 ) {
                ...
                generatedDownload.addProperty("name", "Apache Felix " + singleAddition);
    ...
    public static String getDownloadUrl(String fullVersion) {
        return "https://archive.apache.org/dist/felix/org.apache.felix.main.distribution-" + fullVersion + ".zip";
    }
    ...
    public static String[] findAllVersionsFromWebArchive() {
        ...
        String prefix = "org.apache.felix.main.distribution-";
        String suffix = ".zip";
        try {
            URL oracle = new URL("https://archive.apache.org/dist/felix/");
            BufferedReader in = new BufferedReader(new InputStreamReader(oracle.openStream()));
            String inputLine;
            while ((inputLine = in.readLine()) != null) {
                if( inputLine.contains("compressed.gif")) {
                    ...
                    if( ind > -1 ) {
                        ...
                        if( end > -1 ) {
                            ...
                            if( sub2.startsWith(prefix)) {
                                String sub3 = sub2.substring(prefix.length());
                                if( sub3.endsWith(suffix)) {
                                    sub3 = sub3.substring(0, sub3.length() - suffix.length());
                                    if( !sub3.contains("-")) {
                                        collector.add(sub3);
                                    }
                                }
