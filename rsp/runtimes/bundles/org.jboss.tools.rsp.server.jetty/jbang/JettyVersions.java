...
public class JettyVersions {
    ...
    private static void ensureVersionAdded(String singleAddition, JsonObject serversGson) {
        ...
        String serverTypeKey = "org.jboss.ide.eclipse.as.server.jetty." + major(singleAddition) + "x";
        ...
        if( typeObj == null ) {
            ...
            generatedType.addProperty("template", "jetty.template");
            JsonObject generatedDiscoveries = new JsonObject();
            String majorDiscoveryKey = "jetty." + major(singleAddition) + ".x";
            JsonObject majorDiscoveryVal = new JsonObject();
            majorDiscoveryVal.addProperty("discoveryType", "jarManifest");
            majorDiscoveryVal.addProperty("name", "Jetty " + major(singleAddition) + ".x");
            majorDiscoveryVal.addProperty("nameFile", "start.jar");
            majorDiscoveryVal.addProperty("nameKey", "Bundle-SymbolicName");
            majorDiscoveryVal.addProperty("nameRequiredPrefix", "org.eclipse.jetty.start");
            majorDiscoveryVal.addProperty("versionFile", "start.jar");
            ...
            generatedDownloads.addProperty("downloadProviderId", "jetty" + major(singleAddition) + ".x");
            generatedType.add("downloads", generatedDownloads);

            JsonObject generatedInnerType = new JsonObject();
            generatedInnerType.addProperty("name", "Jetty " + major(singleAddition) + ".x");
            String descString = "A server adapter capable of discovering and controlling a Jetty " + major(singleAddition) + ".x runtime instance.";
        ...
        String dlKey = "jetty-" + singleAddition;
        if( downloadsForType.get(dlKey) == null ) {
            ...
            if( size > -1 ) {
                ...
                generatedDownload.addProperty("name", "Jetty " + singleAddition);
    ...
    public static String getDownloadUrl(String fullVersion) {
        return "https://repo1.maven.org/maven2/org/eclipse/jetty/jetty-home/" + fullVersion + "/jetty-home-" + fullVersion + ".zip";
    }
    ...
    public static String[] findAllVersionsFromWebArchive() {
        ...
        try {
            URL oracle = new URL("https://repo1.maven.org/maven2/org/eclipse/jetty/jetty-home/");
                    ...
                    if( end > -1 ) {
                        ...
                        if( segments.length == 3 ) {
                            ...
                            if( major.matches("-?\\d+") && minor.matches("-?\\d+")) {
                                // 11 minimum
                                if( Integer.parseInt(major) >= 11 && !segments[2].contains("-")) {
